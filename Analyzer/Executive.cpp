////////////////////////////////////////////////////////////////////////////////
// Executive.cpp - Directs Code Analysis and Code Publisher		              //
//																		      //
...
* Package Operations:
* -------------------
* This package contains the main function that performs the dependency analysis
*
* Required Files:
* ---------------
* Executive.h, Parser.h, FileSystem.h, FileMgr.h, ActionsAndRules.h, ConfigureParser.h, 
* AbstrSynTree.h, Logger.h, Utilities.h
* Build Process:
* --------------
...
void CodeAnalysisExecutive::executeMain(int argc, char* argv[])
{
	...
	try {
		...
		if (!succeeded) { std::cout << "Unable to process command line args"; }
	...
	//	Rslt::write("\n");
		...
		exec.stopLogger();*/
		ta.doTypeAnal();
		ta.doDepAnal(argv[argc - 1]);
		std::string path = "../ServerRepo/";
		std::string command("start \"\" \"" + path + argv[argc - 1] + "\"");
...
#ifdef TEST_EXECUTIVECPP



int main(int argc, char* argv[]){	// test executive for CodeAnalyzer
  using namespace CodeAnalysis;
  CodeAnalysisExecutive exec;
  TypeAnal ta;
  try {
	title("REQUIREMENT #1 - Using Visual Studio 2015 and its C++ Windows Console Projects");
	std::cout << std::endl;
	title("REQUIREMENT #2 - Using the C++ standard library's streams for all I/O and new and delete for all heap-based memory management");
	std::cout << std::endl;
	title("REQUIREMENT #3 - Provided a Publisher program that provides for creation of web pages each of which captures the content of a single C++ source code file, e.g., *.h or *.cpp");
	std::cout << "\n The published web pages are stored in the following default directory -> CodePublisher/PublishedFiles/ \n";
    bool succeeded = exec.ProcessCommandLine(argc, argv);
    ...
    Rslt::write("\n");	std::ostringstream out;
	exec.stopLogger();
	title("\n  REQUIREMENT #4 - Provided the facility to expand or collapse class bodies, methods, and global functions using JavaScript and CSS properties");
	std::cout << "\n This requirement can be observed by checking any of the published pages which displays a small button to expand or collapse the above mentioned scopes\n";
	...
	std::cout << "\n The default location for the .css and .js file -> CodePublisher/PublishedFiles/ \n";
	title("\n  REQUIREMENT #6 - Embedded in each web page's <head> section, links to the style sheet and JavaScript file");
	title("REQUIREMENT #7 - Embedded HTML5 links to dependent files with a label, at the top of the web page");
	ta.doTypeAnal();
	ta.doDepAnal(argv[argc - 1]);
	title("REQUIREMENT #8 - Develop command line processing to define the files to publish by specifying path and file patterns.");
	std::cout << "\n\n Processing the command line : .. / FilesToPublish *.h *.cpp / m index.htm \n";
	title("REQUIREMENT #9 - Demonstrating the CodePublisher functionality by publishing all the important packages in this Project #3");
	std::cout << "\n The files published from important packages -> CodePublisher/FilesToPublish/ \n";
	title("REQUIREMENT #10 - Includes an automated unit test suite that demonstrates meeting all the requirements of this project");
	std::cout << "\n A default index page is published which starts up on the default browser \n";
	std::string path = "../PublishedFiles/";	 
	std::string command("start \"\" \"" + path + argv[argc - 1] + "\"");
	std::system(command.c_str());  
	std::cin.get();
  }
